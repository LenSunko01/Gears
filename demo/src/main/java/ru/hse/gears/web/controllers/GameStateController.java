...
public class GameStateController {
    ...
    DeferredResult<ResponseEntity<GameState>> gameState(
        ...
        ForkJoinPool.commonPool().submit(() -> {
            ...
            getGameIfReady(id, currentPlayer, token, output);
    ...
    DeferredResult<ResponseEntity<String>> updateGameState(
        ...
        ForkJoinPool.commonPool().submit(() -> {
            ...
            updateGame(id, currentPlayer, newGameState, token, output);
    ...
    DeferredResult<ResponseEntity<GameState>> getBoardFromFirstPlayer(
        ...
        ForkJoinPool.commonPool().submit(() -> {
            ...
            getBoardFromFirstPlayer(id, token, output);
    ...
    DeferredResult<ResponseEntity<GameState>> initBoard(
        ...
        ForkJoinPool.commonPool().submit(() -> {
            ...
            initializeBoard(id, currentPlayer, board, token, output);
    ...
    DeferredResult<ResponseEntity<String>> deleteGame(
        ...
        ForkJoinPool.commonPool().submit(() -> {
            ...
            deleteGame(id, currentPlayer, token, output);
    ...
    DeferredResult<ResponseEntity<String>> sendMessage(
        ...
        ForkJoinPool.commonPool().submit(() -> {
            ...
            sendMessage(message, id, currentPlayer, token, output);
    ...
    DeferredResult<ResponseEntity<Message>> getMessage(
        ...
        ForkJoinPool.commonPool().submit(() -> {
            ...
            getMessageIfReady(id, currentPlayer, token, output);
    ...
    private void getGameIfReady(
            Long id,
            GameState.CurrentPlayer currentPlayer,
            String token,
            DeferredResult<ResponseEntity<GameState>> output
    ) {
        ...
        try {
            ...
            output.setErrorResult(ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(e.getMessage()));
            logger.error("Exception while executing GET game request with id " + id + " and player "
    ...
    private void updateGame(
        ...
        try {
            ...
            output.setErrorResult(ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(e.getMessage()));
            logger.error("Exception while executing POST game request with id " + id + " and player "
                    + currentPlayer + " : " + e.getMessage());
        } catch (Exception e) {
            output.setErrorResult(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(e.getMessage()));
    ...
    private void getBoardFromFirstPlayer(
            Long id,
            String token,
            DeferredResult<ResponseEntity<GameState>> output
    ) {
        ...
        try {
            ...
            output.setErrorResult(ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(e.getMessage()));
            logger.error("Exception while executing GET board request with id " + id + " : " + e.getMessage());
        } catch (Exception e) {
            output.setErrorResult(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(e.getMessage()));
            logger.error("Exception while executing GET board request with id " + id + " : " + e.getMessage());
    ...
    private void initializeBoard(
        ...
        try {
            ...
            output.setErrorResult(ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(e.getMessage()));
            logger.error("Exception while executing POST init game request with id " + id + " and player "
                    + currentPlayer + " : " + e.getMessage());
        } catch (Exception e) {
            output.setErrorResult(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(e.getMessage()));
    ...
    private void deleteGame(
        ...
        try {
            ...
            output.setErrorResult(ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(e.getMessage()));
            logger.error("Exception while executing DELETE game request with id " + id + " and player "
                    + currentPlayer + " : " + e.getMessage());
        } catch (Exception e) {
            output.setErrorResult(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(e.getMessage()));
            logger.error("Exception while executing DELETE game request: " + e.getMessage());
        }
    }

    private void sendMessage(Message message, Long id, GameState.CurrentPlayer currentPlayer, String token, DeferredResult<ResponseEntity<String>> output) {
        Lock lock = new ReentrantLock();
        try {
            ...
            output.setErrorResult(ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(e.getMessage()));
            logger.error("Exception while executing POST message request with id " + id + " and player "
                    + currentPlayer + " : " + e.getMessage());
        } catch (Exception e) {
            output.setErrorResult(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(e.getMessage()));
    ...
    private void getMessageIfReady(Long id, GameState.CurrentPlayer currentPlayer, String token, DeferredResult<ResponseEntity<Message>> output) {
        Lock lock = new ReentrantLock();
        try {
            lock = lockMessageMap.get(id);
            lock.lock();
            ...
            output.setErrorResult(ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(e.getMessage()));
            logger.error("Exception while executing GET message request with id " + id + " and player "
                    + currentPlayer + " : " + e.getMessage());
        } catch (Exception e) {
            output.setErrorResult(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(e.getMessage()));
